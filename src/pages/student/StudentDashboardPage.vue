<template>
  <PrivateLayout>
    <div class="relative space-y-8 rounded-2xl p-6 bg-surface shadow-md border border-white/10 overflow-hidden">
      ...
      <!-- Header -->
      <div class="relative">
        <h1 class="text-2xl font-semibold tracking-tight text-text-primary sm:text-3xl">{{ t('studentDashboard.greeting', { name: displayName }) }}</h1>
        ...
        <AppCard>
          <div class="flex items-start gap-4">
            ...
            <div class="min-w-0">
              ...
              <p class="mt-1 text-2xl font-semibold tabular-nums tracking-tight text-text-primary sm:text-3xl">{{ dashboardData?.totalBookings ?? 0 }}</p>
            </div>
          </div>
        </AppCard>
      ...
      <div class="relative overflow-hidden rounded-2xl bg-surface/60 p-6 ring-1 ring-border-brand/40 shadow-lg shadow-brand/5">
        <div class="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-brand/10 blur-3xl" />
        <div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div class="flex-1">
            <div class="flex items-center gap-3 mb-2">
              <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-brand to-accent text-on-brand shadow-sm">
          ...
          <div class="w-full md:w-auto shrink-0 flex items-center justify-center p-4 rounded-xl bg-surface/40 ring-1 ring-border-subtle">
            <div class="text-center">
              ...
              <div class="flex gap-2">
                <div class="h-10 w-10 flex items-center justify-center rounded-full bg-warning-soft ring-1 ring-warning-border" title="First Booking">🌟</div>
                <div class="h-10 w-10 flex items-center justify-center rounded-full bg-success-soft ring-1 ring-success-border" title="Completed 3 Lessons">🚀</div>
      ...
      <div v-if="(dashboardData?.completedBookings ?? 0) === 0" class="relative overflow-hidden rounded-2xl bg-surface/60 p-10 text-center ring-1 ring-border-brand/40 shadow-sm">
        ...
        <div class="relative z-10 max-w-md mx-auto flex flex-col items-center">
          ...
          <button
              class="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-2xl bg-brand px-6 py-2 pb-2.5 font-semibold text-on-brand shadow-sm transition hover:-translate-y-0.5 hover:bg-brand-hover active:scale-[0.98]"
      ...
      <div class="relative space-y-4">
        ...
        <InfoPanel v-else-if="appStatus && appStatus.status === 'APPROVED'" variant="success">
          ...
          <button
              class="mt-4 inline-flex rounded-xl bg-brand px-5 py-2.5 text-sm font-semibold text-on-brand transition hover:bg-brand-hover"
        ...
        <InfoPanel v-else-if="appStatus && appStatus.status === 'REJECTED'" variant="error">
          ...
          <button
              class="mt-4 inline-flex rounded-xl bg-danger px-5 py-2.5 text-sm font-semibold text-on-brand transition hover:brightness-110"
              @click="reapply"
          >
